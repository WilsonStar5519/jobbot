...
function openBrowser(url) {
  // README 明確以 Windows 為目標平台，但伺服器本身係跨平台 Node.js，
  // 加返平台判斷同錯誤處理，避免喺 macOS／Linux 開發或測試時，因為 Windows-only 嘅
  // "start" 指令唔存在而觸發未捕捉嘅子程序錯誤。
  const cmd =
    process.platform === "win32" ? `start "" "${url}"` : process.platform === "darwin" ? `open "${url}"` : `xdg-open "${url}"`;
  const child = exec(cmd, (err) => {
    if (err) log(`未能自動開啟瀏覽器，請手動前往 ${url}`);
  });
  child.on("error", () => log(`未能自動開啟瀏覽器，請手動前往 ${url}`));
}
